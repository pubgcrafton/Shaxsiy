...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _premium_client_id_logging_tag = copy.copy(
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?premium$", from_id=659800858)
...
class Modules:
    ...
    async def _junk_collector(self):
        ...
        while True:
            ...
            for module in self.modules:
                commands.update(module.premium_commands)
                inline_handlers.update(module.premium_inline_handlers)
                callback_handlers.update(module.premium_callback_handlers)
                watchers.extend(module.premium_watchers.values())
    ...
    async def _register_modules(
        ...
        with contextlib.suppress(AttributeError):
            _premium_client_id_logging_tag = copy.copy(self.client.tg_id)
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith("<core"):
            self._core_commands += list(
                map(lambda x: x.lower(), list(instance.premium_commands))
            )
        ...
        for _command, cmd in instance.premium_commands.items():
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.premium_commands:
                self.add_alias(alias, cmd)

        for name, func in instance.premium_inline_handlers.copy().items():
        ...
        for name, func in instance.premium_callback_handlers.copy().items():
    ...
    def register_watcher(self, instance: Module):
        ...
        for _watcher in instance.premium_watchers.values():
    ...
    async def _decline(
        ...
        self._db.set(
            "premium.main",
            "declined_joins",
            list(set(self._db.get("premium.main", "declined_joins", []) + [channel.id])),
        )
        event.status = False
        event.set()
        await call.edit(
    ...
    async def _request_join(
        ...
        if channel.id in self._db.get("premium.main", "declined_joins", []):
            if assure_joined:
                raise LoadError(
        ...
        _module.strings._base_strings["_premium_internal_request_join"] = (
        ...
        _module.strings_ru["_premium_internal_request_join"] = (
        ...
        await self.inline.bot.send_animation(
            ...
            caption=_module.strings("_premium_internal_request_join"),
        ...
        _module.premium_wait_channel_approve = (
        ...
        with contextlib.suppress(AttributeError):
            delattr(_module, "premium_wait_channel_approve")
    ...
    def get_prefix(self) -> str:
        return self._db.get("premium.main", "command_prefix", ".")
    ...
    async def _mod_import_lib(
        ...
        *,
        ...
        if re.search(r"# ?scope: ?premium_min", code):
            ver = tuple(
                map(
                    int,
                    re.search(r"# ?scope: ?premium_min ((\d+\.){2}\d+)", code)[1].split(
                        "."
                    ),
                )
            )
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Premium version {'{}.{}.{}'.format(*ver)}+"
                    )
                )

        module = f"premium.libraries.{url.replace('%', '%%').replace('.', '%d')}"
        ...
        if (
            ...
            and self._db.get("premium.main", "stats", True)
            and url is not None
