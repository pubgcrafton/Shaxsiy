...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.premium_me.id
        self._cached_username = (
            self._client.premium_me.username.lower()
            if self._client.premium_me.username
            else str(self._client.premium_me.id)
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("premium-")
            and message.chat.title != "premium-logs"
        ):
            if not watcher:
    ...
    async def future_dispatcher(
        ...
        *args,
        ...
        _premium_client_id_logging_tag = copy.copy(self.client.tg_id)  # skipcq
        try:
            await func(message)
