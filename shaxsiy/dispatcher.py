...
class CommandDispatcher:
    def __init__(
        ...
        self._me = self._client.shaxsiy_me.id
        self._cached_username = (
            self._client.shaxsiy_me.username.lower()
            if self._client.shaxsiy_me.username
            else str(self._client.shaxsiy_me.id)
        )
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("shaxsiy-")
            and message.chat.title != "shaxsiy-logs"
        ):
            if not watcher:
    ...
    async def future_dispatcher(
        ...
        *args,
        ...
        _shaxsiy_client_id_logging_tag = copy.copy(self.client.tg_id)  # skipcq
        try:
            await func(message)
