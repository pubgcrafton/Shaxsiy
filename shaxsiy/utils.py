...
async def answer(
    ...
    **kwargs,
    ...
    :param message: Message to answer to. Can be a tl message or shaxsiy inline object
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        ...
        if len(text) >= 4096 and not hasattr(message, "shaxsiy_grepped"):
            try:
...
def get_platform_emoji(client: typing.Optional[CustomTelegramClient] = None) -> str:
    ...
    if client and (
        client.loader._db.get("shaxsiy.translations", "lang", False) or ""
    ).startswith("ar"):
...
def get_commit_url() -> str:
    """
    Get current Shaxsiy git commit url
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/pubgcrafton/Shaxsiy/commit/{hash_}" if hash_ else "",
    )
