...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.premium_caller or client_id == record.premium_caller)
        ]
    ...
    async def sender(self):
        ...
        self._exc_queue = {
            client_id: [
                ...
                for item in self.tg_buff
                if isinstance(item[0], PremiumException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_premium_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_premium_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                self.tg_buff += [
                    (
                        PremiumException.from_exc_info(
                            *record.exc_info,
