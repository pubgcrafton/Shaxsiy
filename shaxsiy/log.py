...
class TelegramLogsHandler(logging.Handler):
    ...
    def dumps(
        ...
        return [
            ...
            and (not record.shaxsiy_caller or client_id == record.shaxsiy_caller)
        ]
    ...
    async def sender(self):
        ...
        self._exc_queue = {
            client_id: [
                ...
                for item in self.tg_buff
                if isinstance(item[0], ShaxsException)
    ...
    def emit(self, record: logging.LogRecord):
        try:
            caller = next(
                (
                    frame_info.frame.f_locals["_shaxsiy_client_id_logging_tag"]
                    for frame_info in inspect.stack()
                    if isinstance(
                        getattr(getattr(frame_info, "frame", None), "f_locals", {}).get(
                            "_shaxsiy_client_id_logging_tag"
                        ),
                        int,
                    )
                ),
        ...
        if record.levelno >= self.tg_level:
            if record.exc_info:
                ...
                self.tg_buff += [
                    (
                        ShaxsiyException.from_exc_info(
                            *record.exc_info,
