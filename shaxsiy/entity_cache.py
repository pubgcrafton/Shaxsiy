...
def install_entity_caching(client: TelegramClient):
    ...
    async def new(entity: EntityLike):
        ...
        _shaxsiy_client_id_logging_tag = copy.copy(client._tg_id)  # skipcq
        ...
        if hashable_entity and hashable_entity in client._shaxsiy_cache:
            logger.debug(
                f"Using cached entity {entity} ({type(client._shaxsiy_cache[hashable_entity].entity).__name__})"
            )
            return client._shaxsiy_cache[hashable_entity].entity

        resolved_entity = await old(entity)

        if resolved_entity:
            cache_record = CacheRecord(hashable_entity, resolved_entity)
            client._shaxsiy_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                client._shaxsiy_cache[resolved_entity.id] = cache_record

            if getattr(resolved_entity, "username", None):
                logger.debug(
                    f"Saved resolved_entity username @{resolved_entity.username} to cache"
                )
                client._shaxsiy_cache[f"@{resolved_entity.username}"] = cache_record

        return resolved_entity

    async def cleaner(client: TelegramClient):
        while True:
            for record, record_data in client._shaxsiy_cache.copy().items():
                if record_data.expired():
                    del client._shaxsiy_cache[record]
