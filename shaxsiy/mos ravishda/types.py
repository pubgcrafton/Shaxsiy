...
class InlineQuery(AiogramInlineQuery):
        ...
        return [
            InlineQueryResultArticle(
                ...
                input_message_content=InputTextMessageContent(
                    "😶‍🌫️ <i>Bu yerda hech narsa yo'q ...</i>",
                    parse_mode="HTML",
                ),
                thumb_url=thumb_url,
                thumb_width=128,
    ...
    async def e400(self):
        await self.answer(
            self._get_res(
                "🚫 400",
                "Noto'g'ri so'rov. To'g'ri argumentlarni topshirishingiz kerak, modulga amal qiling"
                 "hujjatlar",
    ...
    async def e403(self):
        await self.answer(
            self._get_res(
                "🚫 403",
                "Sizda bu natijaga kirish uchun ruxsat yo‘q",
    ...
    async def e426(self):
        await self.answer(
            self._get_res(
                "🚫 426",
                "Ushbu soʻrovni yuborishdan oldin Shaxsiy userbot maʼlumotlarini yangilashingiz kerak",
    ...
    async def e500(self):
        await self.answer(
            self._get_res(
                "🚫 500",
                "So‘rovni qayta ishlashda ichki userbot xatosi.  Batafsil ma'lumot jurnallarda",
