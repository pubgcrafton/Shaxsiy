...
class TokenObtainment(InlineUnit):
    async def _create_bot(self):
        ...
        # We create new bot
        logger.info("Foydalanuvchida bot yo‘q, yangisini yaratishga urinmoqda")
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            if self._db.get("shaxsiy.inline", "custom_bot", False):
                username = self._db.get("shaxsiy.inline", "custom_bot").strip("@")
                username = f"@{username}"
                try:
            ...
            for msg in [
                f"💢 Shaxsiy Userbot of {self._name}"[:64],
                username,
                "/setuserpic",
                username,
            ]:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if self._db.get(
                        "shaxsiy.inline", "custom_bot", False
                    ) and self._db.get(
                        "shaxsiy.inline", "custom_bot", False
                    ) != button.text.strip(
                        "@"
                    ...
                    if not self._db.get(
                        ...
                        "custom_bot",
                        False,
                    ) and not re.search(r"@shaxsiy_[0-9a-zA-Z]{6}_bot", button.text):
                        continue
                    ...
                    self._db.set("shaxsiy.inline", "bot_token", token)
                    self._token = token
