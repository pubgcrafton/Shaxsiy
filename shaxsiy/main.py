...
class Shaxsiy:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("shaxsiy-") and f.endswith(".session"),
    ...
    async def save_client_session(self, client: CustomTelegramClient):
        if hasattr(client, "_tg_id"):
            ...
            client.shaxsiy_me = me

        session = SQLiteSession(
            os.path.join(
                self.arguments.data_root or BASE_DIR,
                f"shaxsiy-{telegram_id}",
            )
        )
        ...
        client.shaxsiy_db = database.Database(client)
        await client.shaxsiy_db.init()

    async def _web_banner(self):
        """Shows web banner"""
        logging.info("✅ Veb rejimi konfiguratsiyaga tayyor")
