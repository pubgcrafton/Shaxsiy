...
class Premium:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("premium-") and f.endswith(".session"),
    ...
    async def save_client_session(self, client: CustomTelegramClient):
        if hasattr(client, "_tg_id"):
            ...
            client.premium_me = me

        session = SQLiteSession(
            os.path.join(
                self.arguments.data_root or BASE_DIR,
                f"premium-{telegram_id}",
            )
        )
        ...
        client.premium_db = database.Database(client)
        await client.premium_db.init()
