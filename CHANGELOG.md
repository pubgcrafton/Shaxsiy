...
## 🌑 Shaxsiy Userbot 1.5.0

- Fix `on_change` param processing in config
- Fix `shaxsiy.types.CoreOverwriteError`
...
- Add `shaxsiy.validators.MultiChoice`
- Add `utils.get_args_html` to get arguments of command with HTML
- Add switch to mute @BotFather only once in shaxsiy inline
...
- Change inline query placeholder to `user@shaxsiy:~$` + legacy migration
...
## 🌑 Shaxsiy Userbot 1.4.2

- Fix authorization error

## 🌑 Shaxsiy Userbot 1.4.1

- Create new type :obj:`shaxsiy.tl_cache.CustomTelegramClient` to avoid monkeypatching
- Add `ttl` param for :method:`shaxsiy.utils.asset_channel`
- Add support for custom branches (e.g. for beta testers and users, who rolled back)
- Fix automatic modules reactions
- Fix :method:`shaxsiy.inline.utils.Utils._find_caller_sec_map`
- Fix the targeted security rules without time limit
- Require Shaxsiy-TL >= 1.24.9
- Refactor `document_id` of custom emojis
- Refactor validators to be classes, not functions
- Refactor typehints

## 🌑 Shaxsiy Userbot 1.4.0

- Publish shaxsiy telethon fork and migrate to it in requirements, thereby fixing the deployment error on Heroku
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `ShaxsiySecurity`
- Fix `.uninstall_shaxsiy` being accessible by sudo
- Fix `utils.find_caller` for :method:`shaxsiy.inline.utils.Utils._find_caller_sec_map`
- Fix `.eval`
...
- Add platform-specific Shaxsiy emojis to premium users
...
## 🌑 Shaxsiy Userbot 1.3.3

- Fix typo, which broke `client.get_fulluser`

## 🌑 Shaxsiy Userbot 1.3.2
...
- Move `shaxsiy._types` to `shaxsiy.types` with legacy support
- Remake all core modules to decorators
- Force custom shaxsiy telethon installation with 144 layer support
- Add animated emojis to core modules strings
- Add trigger to toggle the appearence of custom emojis

## 🌑 Shaxsiy Userbot 1.3.0
...
- Add support for `# scope: shaxsiy_min` metatag in libraries
...
- Update quickstart
- New fields in ShaxsiyInfo
- Remove suffix `Shaxsiy` if user specified it somewhere below
...
- Add `client.force_get_entity` to bypass Shaxsiy Cacher
...
- Fix translation issue in ShaxsiyConfig
- Fix `.dump`
...
- Add banner with Shaxsiy installation status
- Reorder config saving in web
- Fix `Unauthorized` error on Heroku

## 🌑 Shaxsiy Userbot 1.2.3
...
- Return :obj:`InlineMessage` in `shaxsiy.inline.gallery.Gallery.gallery`
- Fix typo in docstring

## 🌑 Shaxsiy Userbot 1.2.2
...
- Add heroku waker
- Make `shaxsiy-` app naming optional

## 🌑 Shaxsiy Userbot 1.2.0
...
- Merge `shaxsiy_logger.py` with `test.py`
- Localization
...
## 🌑 Shaxsiy Userbot 1.1.25

- Add separate messages on restart and full\_restart. Second one is shown, when all modules are loaded
- Replace .uninstall\_shaxsiy with full uninstallation (remove bot, asset chats and folder)
...
- Add automatic shaxsiy folder processing
- Add avatars to all official repo modules, which require asset chats (and add them to shaxsiy folder)
...
## 🌑 Shaxsiy Userbot 1.1.1

- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/pubgcrafton/Shaxsiy/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Shaxsiy Userbot 1.1.0

- Make inline commands' docstrings translatable
- Introducing ShaxsiyDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor ShaxsiySecurity, especially the bounding mask control

## 🌑 Shaxsiy Userbot 1.0.28
...
## 🌑 Shaxsiy Userbot 1.0.23

- Add ability to disable ShaxsiyDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Shaxsiy Userbot 1.0.22

Add ShaxsiyDL module to use inline download buttons in verified channels

## 🌑 Shaxsiy Userbot 1.0.21
...
## 🌑 Shaxsiy Userbot 1.0.8

- Code refactoring
- Minor bug fixes
- Rewrite security

## 🌑 Shaxsiy Userbot 1.0.7
